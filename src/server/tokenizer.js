'use strict';
// tokenizer.js
const { KEYWORDS_MAP, isPredefinedIdentifier } = require('./languageData');
    ...
    while (i < len) {
        try { // ─────────────────── Process Next Token ──────────────────────────────────────
            ...
            if (/[a-zA-Z_]/.test(text[i])) {
                ...
                const kw = KEYWORDS_MAP.get(lower);
                ...
                // const type = KEYWORDS_MAP.has(lower) ? TT.KEYWORD : TT.IDENT;
            ...
            i++;
            message = `tokenize had an error: ${errResult.message}`;
            logger.err(errResult, message);
            this.warningAtStart(message);            
        }
    }
    // ─────────────────────────────────────────────────────────────────────────────────
