...
Dismount (and remount) the volume. This will prompt Windows to do all the normal housekeeping tasks that are done when mounting a volume, usually only done when booting the computer, such as a quick scan for errors, and on NTFS volumes to re-allocate the NTFS reserved zones.
...
_Source HTML: `VolumeActions-DismountVolume.html`_
