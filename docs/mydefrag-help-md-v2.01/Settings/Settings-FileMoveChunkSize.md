...
Set the file-move chunk size. Minimum chunk size is the number of bytes per cluster (depends on how the harddisk was formatted), maximum chunk size is 1 gigabyte (imposed by the Microsoft defragmentation API).
...
|                                                                 |
|-----------------------------------------------------------------|
| `FileMoveChunkSize(`[`NUMBER`](../Scripts/Scripts-NUMBER.md)`)` |
...
_Source HTML: `Settings-FileMoveChunkSize.html`_
