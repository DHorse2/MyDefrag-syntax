...
## Structured Response And Execution Records

When completing a task, AI tools should provide a structured response that is useful to both humans and downstream automation.

For normal task completion, include:

1. Summary
2. Files changed
3. Validation performed
4. Risks or unresolved issues
5. Recommended next steps, when applicable

For Codex, coding agents, MCP agents, scripts, harnesses, and other agents that perform file changes, command execution, diagnostics, review, or generation, also produce an execution record when practical.

The execution record should follow `D:\AI\.AI\Standards\Execution_Record_Standard.md`.

At minimum, the execution record should capture:

- Run or task identifier, if available.
- Agent name and agent type.
- Prompt or task that initiated the work.
- Working directory and target paths.
- Files read, created, modified, moved, or deleted.
- Commands or tool calls executed.
- Exit codes and errors when available.
- Diagnostics, failed attempts, and recovery actions.
- Tests or validation performed.
- Generated artifacts and their paths.
- Final outcome and unresolved follow-up items.

Do not expose hidden chain-of-thought. Record observable engineering evidence, explicit decisions, assumptions, validation, and outcomes instead.
