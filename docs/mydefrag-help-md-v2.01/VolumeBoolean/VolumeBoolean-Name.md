...
Select the volume if it's name matches the STRING. The string may contain wildcard characters "\*" (zero or more characters) or "?" (a single character).
...
- See the [![ \* ](../img/Bullit.gif) **Label**](VolumeBoolean-Label.md) volumeboolean to select volumes by their label.

### Syntax

|                                                    |
|----------------------------------------------------|
| `Name(`[`STRING`](../Scripts/Scripts-STRING.md)`)` |
...
[![ \* ](../img/Bullit.gif) **Label**](VolumeBoolean-Label.md)  
...
_Source HTML: `VolumeBoolean-Name.html`_
